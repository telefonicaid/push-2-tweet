...
/**
 * Generates the status to be tweeted from the received request
 * @param request The received request
 * @return {string} The status to be tweeted
 */
function getStatus(request) {
  var statusPosition,
      status;
  if (p2tConfig.TWITTER_STATUS_LIST && p2tConfig.TWITTER_STATUS_LIST.length) {
    statusPosition = Math.floor(Math.random() * p2tConfig.TWITTER_STATUS_LIST.length);
    status = p2tConfig.TWITTER_STATUS_LIST[statusPosition];
    status = status.replace(/#button/g, request.push2Tweet.operationDescriptor.button).
           replace(/#action/g, request.push2Tweet.operationDescriptor.action).
           replace(/#extra/g, request.push2Tweet.operationDescriptor.extra).
           replace(/#date/g, new Date());
  } else {
    status = '(' + new Date() + ') Button with id: \'' + request.push2Tweet.operationDescriptor.button + '\'' +
...
/**
 * hapi's on internal-request event handler
 * @param request The received request
 * @param event The event object
 * @param tags The tags array
 */
function onInternalRequest(context, request, event, tags) {
  if (tags.error) {
...
function start(host, port, callback) {
  ...
  server.on('log', onLog.bind(null, context));

  server.on('request-internal', onInternalRequest.bind(null, context));
