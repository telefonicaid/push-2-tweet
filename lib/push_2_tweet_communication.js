...
/**
 * Checks that a request sent to the Push 2 Tweet is well-formed and returns its payload as a JSON object
 * @param {object} request The request sent to the Push 2 Tweet
 * @param {function} callback Callback to be called with the result of the parsing
 * @return {*} This function does not return anything valuable
 */
function parseRequest(request, callback) {
  ...
  if (validation.valid) {
    ...
    return process.nextTick(callback.bind(null, null, payloadJSON));
  } else {
 ...
 * @param {function} callback Callback to be called once the function completes its execution
 */
function getOperationDescriptor(request, callback) {
  parseRequest(request, function(err, payloadJSON) {
    if (err) {
      return process.nextTick(callback.bind(null, err));
    } else {
...
/**
 * Notifies the result of the operation using the callback property passed in the request
 * @param {object} request The received request
 * @param {object} response The response to be notified
 */
function notifyResult(request, response) {
  var requestOptions = {
    method: 'POST',
 ...
 * @param err Error in case an error occured during the processing
 * @param request The received request
 * @param reply hapi's reply() function
 * @param callback A callback to notify when the operation has finished
...
function respond2Request(err, request, reply, callback) {
  ...
  if (err) {
    ...
    if (request.push2Tweet.operationDescriptor.callback) {
      reply();
      // In case the request include a callback property in its payload, notify the callback after some delay
      setTimeout(
        notifyResult.bind(null, request, response),
